...
const D = "hour", Y = "minute", T = "second", S = "millisecond", l = {
  parse_duration(n) {
  ...
  format(n, t = "YYYY-MM-DD HH:mm:ss.SSS", e = "en") {
    ...
    return Object.keys(h).sort((_, c) => c.length - _.length).forEach((_) => {
      d.includes(_) && (d = d.replaceAll(_, `$${g.length}`), g.push(h[_]));
    }), g.forEach((_, c) => {
      d = d.replaceAll(`$${c}`, _);
    }), d;
  },
  diff(n, t, e = "day") {
    ...
    return _ += o % 30 / 30, h = g * 12 + _, n.getDate() < t.getDate() && h--, d = h / 12, e.endsWith("s") || (e += "s"), Math.round(
      {
        milliseconds: i,
        seconds: s,
        minutes: a,
...
class q {
  ...
  calculate_path() {
    ...
    if (this.to_task.$bar.getX() <= this.from_task.$bar.getX() + this.gantt.options.padding) {
      ...
      g < 0 && (g = 0, o = this.gantt.options.padding / 2, d = a ? -o : o);
      const _ = this.to_task.$bar.getY() + this.to_task.$bar.getHeight() / 2 - d, c = this.to_task.$bar.getX() - this.gantt.options.padding;
      this.path = `
                M ${t} ${i}
                v ${g}
                a ${o} ${o} 0 0 1 ${-o} ${o}
...
class F {
  ...
  calculate_progress_width() {
    ...
    const r = this.x + s, a = this.gantt.config.ignored_positions.reduce((h, d) => h + (d >= this.x && d < r), 0) * this.gantt.config.column_width;
    s += a;
  ...
  update_label_position_on_horizontal_scroll({ x: t, sx: e }) {
    ...
    let o = this.$bar.getX() + this.$bar.getWidth(), h = s.getX() + t, d = r && r.getX() + t || 0, g = r && r.getBBox().width + 7 || 7, _ = h + s.getBBox().width + 7, c = e + i.clientWidth / 2;
    s.classList.contains("big") || (_ < o && t > 0 && _ < c || h - g > this.$bar.getX() && t < 0 && _ > c) && (s.setAttribute("x", h), r && (r.setAttribute("x", d), a.setAttribute("x", d)));
  }
  date_changed() {
    let t = !1;
  ...
  compute_start_end_date() {
    ...
    const s = t.getWidth() / this.gantt.config.column_width, r = l.add(
      i,
  ...
  compute_x() {
    ...
    let r = l.diff(e, i, this.gantt.config.unit) / this.gantt.config.step * t;
    this.x = r;
  }
  compute_y() {
...
class I {
  ...
  show({ x: t, y: e, task: i, target: s }) {
    ...
    let r = this.popup_func({
      ...
      set_title: (d) => this.title.innerHTML = d,
      get_subtitle: () => this.subtitle,
      set_subtitle: (d) => this.subtitle.innerHTML = d,
      get_details: () => this.details,
      set_details: (d) => this.details.innerHTML = d,
      add_action: (d, g) => {
        let _ = this.gantt.create_el({
          classes: "action-btn",
          type: "button",
          append_to: this.actions
        });
        typeof d == "function" && (d = d(i)), _.innerHTML = d, _.onclick = (c) => g(i, this.gantt, c);
      }
    });
    if (r === !1) return;
    r && (this.parent.innerHTML = r), this.actions.innerHTML === "" ? this.actions.remove() : this.parent.appendChild(this.actions), this.parent.style.zIndex = 1005, this.parent.style.left = t + 10 + "px", this.parent.style.top = e - 10 + "px", this.parent.classList.remove("hide");
    const a = $("#tasks-gantt")[0], o = this.parent.getBoundingClientRect(), h = a.getBoundingClientRect();
    if (o.bottom >= h.bottom) {
      let d = e - (o.bottom - h.bottom) - 20;
      d > 0 && (this.parent.style.top = d + "px");
    }
  }
  hide() {
...
function O(n, t, e) {
  let i = l.add(n, 6, "day"), s = i.getMonth() !== n.getMonth() ? "D MMM" : "D", r = !t || n.getMonth() !== t.getMonth() ? "D MMM" : "D";
  return `${l.format(n, r, e)} - ${l.format(i, s, e)}`;
}
const b = [
  {
    name: "Hour",
    ...
    lower_text: "HH",
    upper_text: (n, t, e) => !t || n.getDate() !== t.getDate() ? l.format(n, "D MMMM", e) : "",
  ...
  },
  {
    ...
    date_format: "YYYY-MM-DD HH:",
    lower_text: "HH",
    upper_text: (n, t, e) => !t || n.getDate() !== t.getDate() ? l.format(n, "D MMM", e) : "",
  ...
  },
  {
    ...
    date_format: "YYYY-MM-DD HH:",
    lower_text: "HH",
    upper_text: (n, t, e) => !t || n.getDate() !== t.getDate() ? n.getMonth() !== n.getMonth() ? l.format(n, "D MMM", e) : l.format(n, "D", e) : "",
  ...
  },
  {
    ...
    step: "1d",
    lower_text: (n, t, e) => !t || n.getDate() !== t.getDate() ? l.format(n, "D", e) : "",
    upper_text: (n, t, e) => !t || n.getMonth() !== t.getMonth() ? l.format(n, "MMMM", e) : "",
  ...
  },
  {
    ...
    date_format: "YYYY-MM",
    lower_text: "MMMM",
    upper_text: (n, t, e) => !t || n.getFullYear() !== t.getFullYear() ? l.format(n, "YYYY", e) : "",
...
class B {
        ...
        for (let r = new Date(this.gantt_start); r <= this.gantt_end; r.setDate(r.getDate() + 1))
          if (!(this.config.ignored_dates.find(
            ...
            const a = l.diff(
              ...
              this.gantt_start,
              this.config.unit
            ) / this.config.step * this.config.column_width, o = this.grid_height - this.config.header_height, h = l.format(r, "YYYY-MM-DD", this.options.language).replace(" ", "_");
            if (t[r]) {
  ...
  create_el({ left: t, top: e, width: i, height: s, id: r, classes: a, append_to: o, type: h }) {
    ...
    for (let g of a.split(" ")) d.classList.add(g);
    return d.style.top = e + "px", d.style.left = t + "px", r && (d.id = r), i && (d.style.width = i + "px"), s && (d.style.height = s + "px"), o && o.appendChild(d), d;
  }
  make_dates() {
  ...
  get_date_info(t, e) {
    ...
    return r ? typeof r == "string" && (this.config.view_mode.upper_text = (o) => l.format(o, r, this.options.language)) : this.config.view_mode.upper_text = () => "", a ? typeof a == "string" && (this.config.view_mode.lower_text = (o) => l.format(o, a, this.options.language)) : this.config.view_mode.lower_text = () => "", {
      date: t,
      formatted_date: v(
        l.format(
          t,
  ...
  bind_bar_events() {
    ...
    if (p.on(this.$svg, "mousemove", ".bar-wrapper, .handle", (g) => {
      this.bar_being_dragged === !1 && Math.abs((g.offsetX || g.layerX) - d) > 10 && (this.bar_being_dragged = !0);
    }), p.on(this.$svg, "mousedown", ".bar-wrapper, .handle", (g, _) => {
      const c = p.closest(".bar-wrapper", _);
      _.classList.contains("left") ? (s = !0, _.classList.add("visible")) : _.classList.contains("right") ? (r = !0, _.classList.add("visible")) : _.classList.contains("bar-wrapper") && (t = !0), this.popup && this.popup.hide(), e = g.offsetX || g.layerX, g.offsetY || g.layerY, a = c.getAttribute("data-id");
      let u;
      this.options.move_dependencies ? u = [
        a,
        ...this.get_all_dependent_tasks(a)
      ] : u = [a], o = u.map((y) => this.get_bar(y)), this.bar_being_dragged = !1, d = e, o.forEach((y) => {
        const m = y.$bar;
    ...
    p.on(this.$container, "scroll", (g) => {
      ...
      let u;
      i && (u = g.currentTarget.scrollLeft - i), this.current_date = l.add(
        this.gantt_start,
  ...
  bind_bar_progress() {
    ...
    p.on(this.$svg, "mousemove", (o) => {
      ...
      let g = h - t;
      g > s.max_dx && (g = s.max_dx), g < s.min_dx && (g = s.min_dx), s.setAttribute("width", s.owidth + g), p.attr(i.$handle_progress, "cx", s.getEndX()), s.finaldx = g;
  ...
  get_snap_position(t, e) {
    ...
    if (s !== "unit") {
      const { duration: g, scale: _ } = l.parse_duration(s);
      i = l.convert_scales(this.config.view_mode.step, _) / g;
    }
